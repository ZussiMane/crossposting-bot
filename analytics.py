...
class TelegramManager:
    ...
    async def get_dialogs(self, limit: int = 50) -> List[Dict[str, Any]]:
        """Получает список диалогов (только те, где пользователь имеет права администратора)"""
        client = await self.get_client()
        try:
            dialogs = []
            ...
            if not await client.is_user_authorized():
                logger.error("Клиент Telegram не авторизован")
                return []

            # Получаем диалоги
            logger.info("Получение списка диалогов")
            all_dialogs = await client.get_dialogs(limit=limit)
            logger.info(f"Получено {len(all_dialogs)} диалогов")

            for dialog in all_dialogs:
                entity = dialog.entity
                entity_id = getattr(entity, 'id', None)
                entity_title = getattr(entity, 'title', None) or f"{getattr(entity, 'first_name', '')} {getattr(entity, 'last_name', '')}"

                logger.info(f"Проверка диалога: {entity_title} (ID: {entity_id})")
                ...
                logger.info(f"Диалог {entity_title} - канал: {is_channel}, группа: {is_group}")

                if is_channel or is_group:
                    try:
                        # Проверяем права администратора
                        logger.info(f"Проверка прав для {entity_title}")
                        permissions = await client.get_permissions(entity)

                        is_admin = permissions.is_admin or permissions.is_creator
                        logger.info(f"Права администратора для {entity_title}: {is_admin}")

                        # Добавляем только если пользователь админ
                        if is_admin:
                            dialog_info = {
                                'id': entity_id,
                                'title': entity_title,
                            ...
                            logger.info(f"Добавлен диалог: {entity_title}")
                    except Exception as e:
                        logger.warning(f"Ошибка при проверке прав для {entity_title}: {e}")
        ...
        async def publish_post(self, text: str, media_files: List[Dict[str, Any]],
            ...
            # Если не указаны ID чатов, получаем список каналов, где пользователь админ
            if not chat_ids:
                dialogs = await self.get_dialogs()
                chat_ids = [dialog['id'] for dialog in dialogs if dialog['type'] == 'channel']

                if not chat_ids:
                    logger.warning("Не найдено каналов, где пользователь является администратором")
                    return {}

            # Выводим информацию о каналах перед публикацией
            channel_names = []
            for chat_id in chat_ids:
                try:
                    ...
                    channel_names.append(str(chat_id))

            logger.info(f"Публикация в каналы: {', '.join(channel_names)}")
